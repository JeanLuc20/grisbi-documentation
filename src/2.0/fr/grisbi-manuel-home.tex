...
\begin{itemize}
	 \item dans un pavé sur fond gris foncé{\couleur}, l'icône \menu{Grisbi} à gauche, et à droite un \indexword{titre}\index{affichage !titre}\index{titre !page d'accueil} qui permet d'identifier sur quelle comptabilité vous travaillez actuellement, sous la forme \og libellé - Grisbi\fg{} ; vous pouvez définir ce libellé, parmi trois possibilités, dans le menu \menu{Édition - Préférences} (voir le paragraphe \vref{setup-display-addresses-titles}, \menu{Titres}) :
		...
		\end{itemize}
	 \item en-dessous, sur fond gris clair{\couleur}, pour chaque devise séparément, pour tous les comptes et \indexword{groupes de comptes}\index{groupe de comptes}, sous les libellés \menu{Solde rapproché} et \menu{Solde courant} :
		\begin{itemize}
...
\subsection{Menu \menu{Éditer}\label{home-menus-edit}}
...
\begin{itemize}
	...
	\item \menu{Rechercher} : cette fonction renvoie directement à la fenêtre de création/modification des états ; voir le chapitre \vref{reportscreation}, \menu{Création d'un état}, et plus précisément la section \vref{reportscreation-selection}, \menu{Sélection des données} ;
