...
Pour avoir accès aux paramètres de configuration de Grisbi, cliquez sur le menu \menu{Éditer - Préférences}. La fenêtre de configuration s'ouvre, en affichant le contenu du dernier onglet visité. Vous pouvez en changer la position, mais pas la taille. 
...
Grisbi dispose d'un fichier de configuration appelé grisbi.conf, qui se trouve dans l'arborescence du système d'exploitation ; ce chemin est indiqué automatiquement dans le menu \menu{Éditer - Préférences}, voir la section \vref{setup-general-various-general-preferences}, \menu{Paramètres des préférences}.

%{Paramètres des préférences :\label{setup-general-various-general-preferences}}
...
Vous disposez aussi de deux boutons \menu{Ignorer la \gls{casse}}, pour trouver une chaîne contenant à la fois des caractères majuscules et minuscules, et \menu{Utiliser les \gls{expressions regulieres}}.
...
Cette section affiche le chemin complet du fichier de configuration de Grisbi (grisbi.conf) dans votre système (voir le chapitre \vref{setup-conffile}, \menu{Fichier de configuration}).

METTRE GRISBI.CONF DANS L'INDEX

Le bouton \menu{Réinitialisation de la fenêtre des préférences} permet de réinitialiser les dimensions de cette fenêtre à leur valeur par défaut. Cela est utile dans le cas de certaines tailles d'écran, ainsi que si vous avez modifié la taille de la police de caractères et que cela entre en conflit avec celle de la fenêtre.
...
\subparagraph{Choisir la langue :}\index{langue !séparateur}\label{setup-general-various-regional-langue} Choisissez dans la liste déroulante la langue que va utiliser Grisbi. Vous pouvez choisir :

\begin{itemize}
	\item \menu{Langue système} : c'est la langue configurée dans votre système d'exploitation ;
	\item \menu{en\_US} : l'anglais des États-Unis ;
	\item \menu{fr} : le français.
\end{itemize}
...
\begin{itemize}
	\item \menu{Le fichier \file{nom\_de\_votre\_fichier.gsb} est déjà ouvert (\emph{le nom de votre fichier de comptes est affiché automatiquement})} ;
...
\begin{itemize}
	...
	\item \menu{Nom du fichier} : par défaut, c'est le \indexword{nom du fichier}\index{fichier de comptes !nom}\index{nom !fichier de comptes} dans le répertoire courant, sous la forme 	\file{nom\_de\_\_votre\_fichier.gsb}.
\end{itemize}
...
La liste des raccourcis clavier utilise ici les libellés « Alt » et « Primary » pour les touches modificatrices : si la touche \key{ALt} est nommée ainsi sur tous les claviers, d'autres touches modificatrices ont des libellés différents en fonction du clavier ou du système ; voici les correspondances les plus courantes :

\begin{itemize}
	\item pour Windows et Linux, Primary = touche \key{CTRL} ;
	\item pour Mac, Primary = touche \key{Commande} ou touche \key{Cmd}.
\end{itemize}
...
Vous pouvez choisir le \indexword{mode d'affichage} de la liste des opérations\index{affichage !nombre de lignes}\index{affichage !modes}, c'est à dire l'\indexword{ordre des lignes} qui sont affichées pour toutes les opérations, et cela en fonction du nombre de lignes que vous voudrez afficher : 
\begin{itemize}
	\item \menu{Mode d'affichage une ligne} : choisissez entre les lignes 1 à 4 ;
	\item \menu{Mode d'affichage deux lignes} : choisissez dans la liste déroulante entre 1-2, 1-3, 1-4 ;
	\item \menu{Mode d'affichage trois lignes} : choisissez dans la liste déroulante entre 1-2-3, 1-2-4, 1-3-4 ;
\end{itemize}
...
\begin{itemize}
	\item \menu{Ouvrir l'opération sélectionnée avec un simple clic\index{affichage !opérations}} : c'est le choix par défaut ; sinon, on l'ouvre par la touche \key{Entrée} ;
...
\textbf{Note} : la disposition de l'affichage des champs des opérations dans la liste des opérations des comptes est \textbf{totalement indépendante} de celle de  l'affichage des champs dans le formulaire de saisie, qui est elle-même décrite dans la section \vref{setup-form-content}, \menu{Contenu}.
...
Le tableau de prévisualisation représente les 7 colonnes et les lignes permettant d'afficher les \indexword{champs d'une opération}\index{champ d'information !aperçu} ; ces colonnes et lignes sont affichés de la même manière, pour chaque opération, dans la liste des opérations de l'onglet \menu{Opérations} de chaque compte.

Ce tableau comprend donc sept colonnes et quatre lignes, qui définissent 28 \indexword{cellules}\index{cellule !définition}, pouvant donc afficher au maximum 28 champs (voir la section \vref{transactions-list-fields}, \menu{Champs d'information et de saisie}.). Vous pouvez modifier la \indexword{largeur des colonnes}\index{colonne !largeur} en cliquant, dans la ligne des libellés des colonnes, sur le séparateur entre deux d'entre-elles (sans relâcher) et en le déplaçant avec la souris.
...
Cet onglet permet de gérer \ifIllustration les rapprochements effectués antérieurement \refimage{setup-reconciliation-img}, pour tous les comptes de votre fichier de comptes.
...
Vous pouvez ou non cocher la case \menu{Remplissage sensible à la \gls{casse}} \indexword{casse}\index{caractères !casse} des caractères ;
...
%Vous pouvez aussi indiquer si les devises passeront à l'euro lorsque vous choisirez de basculer la tenue d'un compte dans cette monnaie (ceci est probablement obsolète pour beaucoup d'utilisateurs, mais certains peuvent en avoir re besoin).
...
%PRÉFÉRENCES FORMULAIRE DES  OPÉRATIONS À DÉFINIR XXXXXXXXXXX 
...
(voir la section \vref{accounts-type}, \menu{Types de compte de Grisbi} ; par exemple, un compte de caisse n'a pas le virement comme mode de règlement.
...
\begin{enumerate}
	...
	\item dans la section \menu{Données du crédit}, saisissez les données du crédit en vous référant aux documents du crédit que vous avez contracté avec votre organisme bancaire :
		\begin{itemize}
			 ...
			 \item la case \menu{La première échéance est différente des autres} *************ÀÀÀÀ VOIR ;
...
\begin{enumerate}
	...
	\item dans la section \menu{Sources des données historiques} :
		\begin{enumerate}	
			...
			\item définissez la \indexword{période de référence}\index{prévision !période de référence} pour établir les prévisions, grâce à la liste déroulante, qui permet de choisir soit \menu{12 mois glissants}, soit un exercice déjà défini. % leave the following line empty to make sure the left alignment of the following note
